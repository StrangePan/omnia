...
object GraphAlgorithms {
    ...
    fun <T> findAnyCycle(graph: DirectedGraph<T>): Optional<List<T>> {
        ...
        for (directedNode in graph.nodes()) {
            ...
            val iteratorStack: MutableList<Iterator<DirectedGraph.DirectedNode<T>>> = ArrayList.create() // correlates to itemStack. iterators track state.
                    ...
                    if (itemsInStack.contains(nextItem)) {
                        ...
                        return Optional.of(
                                ListAlgorithms.sublistOf(
                                        itemStack, itemStack.indexOf(nextItem).asInt, itemStack.count()))
                    }
    ...
    fun <T> topologicallySort(graph: DirectedGraph<out T>): ImmutableList<T> {
        ...
        val itemsInResult: MutableSet<DirectedGraph.DirectedNode<out T>> = HashSet.create()
        val stack: Stack<Couple<DirectedGraph.DirectedNode<out T>, Iterator<DirectedGraph.DirectedNode<out T>>>> = ArrayStack.create()
        val itemsInStack: MutableSet<DirectedGraph.DirectedNode<out T>> = HashSet.create()
        ...
        for (sourceNode in graph.nodes()) {
            ...
            val subgraphNodes: Set<DirectedGraph.DirectedNode<out T>> = findSubgraph(sourceNode)
        ...
        require(resultList.count() == graph.nodes().count()) { "graph must be acyclic to perform a topological sort" }
        return resultList
    }
    ...
    private fun hasOutgoingEdges(directedNode: DirectedGraph.DirectedNode<*>): Boolean {
        return directedNode.outgoingEdges().isPopulated
    }

    private fun hasNoOutgoingEdges(directedNode: DirectedGraph.DirectedNode<*>): Boolean {
        return !hasOutgoingEdges(directedNode)
    }

    private fun hasIncomingEdges(directedNode: DirectedGraph.DirectedNode<*>): Boolean {
        return directedNode.incomingEdges().isPopulated
    }

    private fun hasNoIncomingEdges(directedNode: DirectedGraph.DirectedNode<*>): Boolean {
