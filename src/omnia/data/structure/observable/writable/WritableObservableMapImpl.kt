...
internal class WritableObservableMapImpl<K, V> : WritableObservableMap<K, V> {
    ...
    override fun removeKey(key: K): Optional<V> {
        synchronized(this) {
            ...
            ) { previousState, _ -> ImmutableSet.of(RemoveFromMap(key, previousState.valueOf(key).orElseThrow())) }
            return removedValue
        }
    }
    ...
    inner class MutationEvent(state: Map<K, V>, operations: Set<MapOperation<K, V>>) : GenericMutationEvent<Map<K, V>, Set<MapOperation<K, V>>>(state, operations), ObservableMap.MutationEvent<K, V>
    ...
    private class AddToMap<K, V>(private val key: K, private val value: V) : ObservableMap.AddToMap<K, V> {
        override fun key(): K {
            return key
        }
    ...
    private class RemoveFromMap<K, V>(private val key: K, private val value: V) : ObservableMap.RemoveFromMap<K, V> {
        override fun key(): K {
            return key
        }
    ...
    private class ReplaceInMap<K, V>(private val key: K, private val replacedValue: V, private val newValue: V) : ObservableMap.ReplaceInMap<K, V> {
        override fun key(): K {
            return key
        }
