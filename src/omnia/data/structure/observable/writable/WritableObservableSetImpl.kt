...
internal class WritableObservableSetImpl<E> : WritableObservableSet<E> {
    ...
    override fun removeUnknownTyped(item: Any?): Boolean {
        return mutateState(
                ...
                { state -> ImmutableSet.copyOf(SetAlgorithms.differenceBetween(state, ImmutableSet.of(item!!))) }
        ) { _, _ ->
    ...
    inner class MutationEvent(state: Set<E>, operations: Set<SetOperation<E>>) : GenericMutationEvent<Set<E>, Set<SetOperation<E>>>(state, operations), ObservableSet.MutationEvent<E>

    private fun generateMutationEventForNewSubscription(): MutationEvent {
        val state: Set<E> = state
        return MutationEvent(state, state.stream().map { item: E -> AddToSet(item) }.collect(Collectors.toSet()))
    }
}