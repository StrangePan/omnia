    ...
    interface MapOperation<K, V> {
        ...
        companion object {
            fun <K, V> justAddToMapOperations(
                    observable: Observable<out MapOperation<K, V>>): Observable<AddToMap<K, V>> {
                return observable.flatMap { mutation: MapOperation<K, V> -> if (mutation is AddToMap<*, *>) Observable.just(mutation as AddToMap<K, V>) else Observable.empty() }
            }

            fun <K, V> justRemoveFromMapOperations(
                    observable: Observable<out MapOperation<K, V>>): Observable<RemoveFromMap<K, V>> {
                return observable.flatMap { mutation: MapOperation<K, V> -> if (mutation is RemoveFromMap<*, *>) Observable.just(mutation as RemoveFromMap<K, V>) else Observable.empty() }
            }

            fun <K, V> justReplaceInMapOperations(
                    observable: Observable<out MapOperation<K, V>>): Observable<ReplaceInMap<K, V>> {
                return observable.flatMap { mutation: MapOperation<K, V> -> if (mutation is ReplaceInMap<*, *>) Observable.just(mutation as ReplaceInMap<K, V>) else Observable.empty() }
            }
        }
    }
